...
static inline void tcp_clear_xmit_timers(struct sock *sk)
{
	...
	if (hrtimer_try_to_cancel(&tcp_sk(sk)->event_timer) == 1)
		__sock_put(sk);
...
/* control/slow paths put last */
	...
	/* tbtcp */
	enum hrtimer_restart (*event_handler)(struct hrtimer *timer);
...
static inline bool tcp_is_tb(struct sock *sk)
{
	struct inet_connection_sock *icsk = inet_csk(sk);
	return strcmp(icsk->icsk_ca_ops->name, "timer_based") == 0;
}
...
enum hrtimer_restart tcp_event_handler(struct hrtimer *timer);
